...
var AnimationState = new Class({
  ...
  load: function (key) {
    ...
    var animKey =
      typeof key === 'string' ? key : GetFastValue(key, 'key', null);
    ...
    if (!anim) {
      ...
      this.showBeforeDelay = GetFastValue(
        key,
        'showBeforeDelay',
        anim.showBeforeDelay,
      );
      this.showOnStart = GetFastValue(key, 'showOnStart', anim.showOnStart);
      this.hideOnComplete = GetFastValue(
        key,
        'hideOnComplete',
        anim.hideOnComplete,
      );
      this.skipMissedFrames = GetFastValue(
        key,
        'skipMissedFrames',
        anim.skipMissedFrames,
      );
  ...
  complete: function () {
    ...
    if (this.nextAnim) {
      ...
      this.nextAnim =
        this.nextAnimsQueue.length > 0 ? this.nextAnimsQueue.shift() : null;
      this.play(key);
    }
    return this.parent;
  },
  ...
  update: function (time, delta) {
    ...
    this.accumulator +=
      delta * this.timeScale * this.animationManager.globalTimeScale;
  ...
  setCurrentFrame: function (animationFrame) {
    ...
    if (gameObject.isCropped) {
      gameObject.frame.updateCropUVs(
        gameObject._crop,
        gameObject.flipX,
        gameObject.flipY,
      );
    ...
    if (gameObject._originComponent) {
      if (animationFrame.frame.customPivot) {
        gameObject.setOrigin(
          animationFrame.frame.pivotX,
          animationFrame.frame.pivotY,
        );
      } else {
    ...
    if (this.isPlaying && this.hasStarted) {
      ...
      if (
        this._pendingStop === 3 &&
        this._pendingStopValue === animationFrame
      ) {
        this.stop();
      }
    }
