...
var InputPlugin = new Class({
  ...
  start: function () {
    ...
    eventEmitter.on(
      SceneEvents.TRANSITION_COMPLETE,
  ...
  processDownEvents: function (pointer) {
    ...
    for (var i = 0; i < currentlyOver.length; i++) {
      ...
      total++;
      gameObject.emit(
        Events.GAMEOBJECT_POINTER_DOWN,
        pointer,
        gameObject.input.localX,
  ...
  processDragThresholdEvent: function (pointer, time) {
    ...
    if (
      distanceThreshold > 0 &&
      DistanceBetween(pointer.x, pointer.y, pointer.downX, pointer.downY) >=
        distanceThreshold
    ) {
      passed = true;
  ...
  processMoveEvents: function (pointer) {
    ...
    for (var i = 0; i < currentlyOver.length; i++) {
      ...
      total++;
      gameObject.emit(
        Events.GAMEOBJECT_POINTER_MOVE,
        pointer,
        gameObject.input.localX,
        gameObject.input.localY,
        _eventContainer,
  ...
  processWheelEvent: function (pointer) {
    ...
    for (var i = 0; i < currentlyOver.length; i++) {
      ...
      total++;
      gameObject.emit(
        Events.GAMEOBJECT_POINTER_WHEEL,
        pointer,
        dx,
        dy,
        dz,
  ...
  processOverEvents: function (pointer) {
    ...
    if (total > 0) {
      ...
      for (var i = 0; i < total; i++) {
        ...
        gameObject.emit(
          Events.GAMEOBJECT_POINTER_OVER,
          pointer,
          gameObject.input.localX,
          gameObject.input.localY,
          _eventContainer,
  ...
  processOverOutEvents: function (pointer) {
    ...
    for (i = 0; i < previouslyOver.length; i++) {
      gameObject = previouslyOver[i];
      if (
        currentlyOver.indexOf(gameObject) === -1 &&
        currentlyDragging.indexOf(gameObject) === -1
      ) {
  ...
  forceState: function (
    ...
    if (gameObject.input && gameObject.input.enabled) {
      ...
      if (
        !_eventData.cancelled &&
        this.isActive() &&
        gameObject.input &&
        gameObject.input.enabled
      ) {
  ...
  queueForInsertion: function (child) {
    if (
      this._pendingInsertion.indexOf(child) === -1 &&
      this._list.indexOf(child) === -1
    ) {
  ...
  setHitArea: function (gameObjects, hitArea, hitAreaCallback) {
    ...
    for (var i = 0; i < gameObjects.length; i++) {
      ...
      var io = !gameObject.input
        ? CreateInteractiveObject(gameObject, hitArea, hitAreaCallback)
        : gameObject.input;
  ...
  setHitAreaFromTexture: function (gameObjects, callback) {
    ...
    for (var i = 0; i < gameObjects.length; i++) {
      ...
      if (width !== 0 && height !== 0) {
        gameObject.input = CreateInteractiveObject(
          gameObject,
  ...
  enableDebug: function (gameObject, color) {
    ...
    if (debug) {
      ...
      debug.preUpdate = function () {
        ...
        debug.setScrollFactor(
          gameObject.scrollFactorX,
          gameObject.scrollFactorY,
        );
  ...
  sortDropZoneHandler: function (childA, childB) {
    if (!childA.parentContainer && !childB.parentContainer) {
      return (
        this.displayList.getIndex(childB) - this.displayList.getIndex(childA)
      );
    } else if (childA.parentContainer === childB.parentContainer) {
      return (
        childB.parentContainer.getIndex(childB) -
        childA.parentContainer.getIndex(childA)
      );
