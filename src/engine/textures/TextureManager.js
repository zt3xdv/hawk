...
var TextureManager = new Class({
  ...
  getBase64: function (key, frame, type, encoderOptions) {
    ...
    if (
      textureFrame &&
      (textureFrame.source.isRenderTexture || textureFrame.source.isGLTexture)
    ) {
      if (!this.silentWarnings) {
    ...
    if (this.checkKey(key)) {
      ...
      if (atlasData) {
        var parse = function (texture, sourceIndex, atlasData) {
          if (
            Array.isArray(atlasData.textures) ||
            Array.isArray(atlasData.frames)
          ) {
