...
var Tilemap = new Class({
  ...
  addTilesetImage: function (
    ...
    if (index === null && this.format === Formats.TILED_JSON) {
      console.warn(
        'Tilemap has no tileset "%s". Its tilesets are %o',
        tilesetName,
        this.tilesets,
      );
      return null;
  ...
  createFromObjects: function (objectLayerName, config, useTileset) {
    ...
    if (!objectLayer) {
      console.warn(
        'createFromObjects: Invalid objectLayerName given: ' + objectLayerName,
      );
      return results;
    }
  ...
  worldToTileXY: function (worldX, worldY, snapToFloor, vec2, camera, layer) {
    ...
    return this._convert.WorldToTileXY(
      worldX,
      worldY,
      snapToFloor,
      vec2,
